...
$deskripsi_matkul_lama = $_POST['deskripsi_matkul_lama'];
$deskripsi_matkul_baru = $_POST['deskripsi_matkul_baru'];
...
if($deskripsi_matkul_baru==$deskripsi_matkul_lama){
  $_SESSION['info'] = 'Gagal Diupdate';
}else{
  $sql = "SELECT * FROM kompetensi_keahlian WHERE deskripsi_matkul = '$deskripsi_matkul_baru'";
  $query 	= mysqli_query($koneksi, $sql);
  // Ketika ditemukan
  if(mysqli_num_rows($query)>0){
    $_SESSION['info'] = 'Gagal Diupdate';
  }else{
    $sql = "UPDATE kompetensi_keahlian SET deskripsi_matkul ='$deskripsi_matkul_baru' WHERE id_kompetensi = '$id_kompetensi'";
    mysqli_query($koneksi, $sql);
    $_SESSION['info'] = 'Diupdate';
  }
}
